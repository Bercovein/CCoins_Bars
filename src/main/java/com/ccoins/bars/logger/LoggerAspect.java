package com.ccoins.bars.logger;

import com.ccoins.bars.exceptions.CustomException;
import com.ccoins.bars.utils.LoggerUtil;
...
public class LoggerAspect {
	...
	@Around("execution(* com.ccoins..repository..*(..)))")
	public Object loggerToRepositoryMethods(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {

		this.loggerInit(proceedingJoinPoint);
		Object result = loggerExecutionToOK(proceedingJoinPoint);
		var log = String.format("%s%s", RESULT_TXT, LoggerUtil.getJson(result));
		LOGGER.info(log);
		return result;
	}
